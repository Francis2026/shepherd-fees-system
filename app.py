...
if "app_initialized" not in st.session_state:
    ...
    st.session_state.initialized_terms = {}  # Track which term/year combos are initialized
...
class FeesManager:
    ...
    def get_term_fees(self, pupil_id, term, year):
        """Get the specific fee for a pupil in a particular term"""
        if supabase is None:
            return None
    ...
    def enroll_pupil(self, name, class_name, term_fees, pupil_type, current_term, current_year):
        ...
        try:
            ...
            # Store the SPECIFIC term fee for this enrollment
    ...
    def enroll_pupil_into_term(self, pupil_id, to_term, to_year, from_term, from_year):
        ...
        try:
            ...
            # Get the CURRENT fee from pupils table (this will be the fee for the NEW term)
            current_fee = pupil.get("term_fees", 0)

            # If pupil is sponsored, fee should be 0
            ...
            # Enroll in term with the SPECIFIC fee for this term
            supabase.table("term_enrollments").insert({
                "pupil_id": pupil_id,
                "term": to_term,
                "year": to_year,
                "term_fees": current_fee,
                "is_active": True,
                "enrolled_at": datetime.datetime.now().isoformat()
            }).execute()
    ...
    def get_class_summary(self, class_name, term, year, include_archived=False):
        """Get class summary using term-specific fees"""
        if supabase is None:
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

        try:
            # Get all enrolled pupils for this term
            result = supabase.table("term_enrollments") \
                .select("pupil_id, term_fees, pupils(*)") \
                .eq("term", term) \
                .eq("year", year) \
            ...
            for item in result.data:
                ...
                if class_name != "All Classes" and pupil.get("class") != class_name:
                    continue

                # Use term-specific fee
                term_fee = item.get("term_fees", pupil.get("term_fees", 0))

                # Get payments for this term
                payments = self.get_ledger(pupil.get("id"), term, year)
                total_paid = sum([p.get("amount", 0) for p in payments if p.get("amount", 0) > 0])
                ...
                summary_data.append({
                    ...
                    "pupil_type": pupil.get("pupil_type"),
                    "term_fees": term_fee,
            ...
            df_archived = df_summary[
                df_summary["status"] == "Archived (Left School)"] if not df_summary.empty else pd.DataFrame()
    ...
    def get_school_wide_summary(self, term, year, include_archived=False):
        """Get school-wide summary using term-specific fees"""
        ...
        try:
            ...
            for item in result.data:
                ...
                all_summaries.append({
                    "No.": len(all_summaries) + 1,
                    "name": pupil.get("name"),
                    "class": pupil.get("class"),
                    "pupil_type": pupil.get("pupil_type"),
                    "Term Fees (UGX)": term_fee,
            ...
            st.error(f"Error getting school summary: {str(e)}")
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
...
def main_app():
                ...
                with col1:
                    if st.button(f"Enroll Selected ({len(selected_pupils)} pupils)", use_container_width=True):
                        for pupil in selected_pupils:
                            manager.enroll_pupil_into_term(
                                pupil['id'],
                                st.session_state.enroll_to_term,
                                st.session_state.enroll_to_year,
                                st.session_state.enroll_from_term,
    ...
    # ------------------- PUPILS & LEDGERS -------------------
        ...
        if not pupils:
            ...
            for pupil in pupils:
                ...
                # Get the term-specific fee for this pupil
                term_fee_for_this_term = manager.get_term_fees(pupil_id, current_term, current_year)
                if term_fee_for_this_term is None:
                    term_fee_for_this_term = term_fees  # Fallback to pupil's default fee
                ...
                # Use term_fee_for_this_term instead of term_fees in calculations
                total_due = debt_amount + term_fee_for_this_term
                ...
                opening_entries = [e for e in ledger_entries if
                                   e.get("amount", 0) == 0 and "carried forward" in str(e.get("description", "")).lower()]
                ...
                if is_archived:
                    ...
                    if previous_balance > 0:
                        expander_title = f"📌 {pupil['name']} — {pupil_type} — ⚠️ Previous Debt: UGX {previous_balance:,.0f} | This Term: UGX {term_fee_for_this_term:,.0f} | Paid: UGX {total_paid_this_term:,.0f} | Balance: UGX {current_balance:,.0f}"
                    elif previous_balance < 0:
                        expander_title = f"📌 {pupil['name']} — {pupil_type} — 💳 Previous Credit: UGX {abs(previous_balance):,.0f} | This Term: UGX {term_fee_for_this_term:,.0f} | Paid: UGX {total_paid_this_term:,.0f} | Balance: UGX {current_balance:,.0f}"
                    else:
                        expander_title = f"📌 {pupil['name']} — {pupil_type} — Fees: UGX {term_fee_for_this_term:,.0f} | Paid: UGX {total_paid_this_term:,.0f} | Balance: UGX {current_balance:,.0f}"
                    ...
                    if all_transactions:
                        ...
                        if payment_entries:
                            ...
                            for entry in payment_entries:
                                ...
                                if receipt_no:
                                    if st.button(f"🖨️ Receipt {str(receipt_no)[-12:]}",
                                        ...
                                        pdf_buffer = generate_pdf_receipt(
                                            ...
                                            term_fees=term_fee_for_this_term,
    ...
    # ------------------- RECORD PAYMENT SECTION (FIXED - NO FUTURE TERMS) -------------------
    elif menu == "Record Payment" and role == "bursar":
        st.markdown("<h1 style='color: #1E3A5F; font-size: 1.5rem;'>💰 Record Payment - All Terms</h1>",
                    unsafe_allow_html=True)
        st.info("✅ **Shows ONLY completed and current terms (no future terms)**")
        ...
        if not all_active_pupils:
            ...
            if not pupil_dicts:
                ...
                if "quick_pay_pupil" in st.session_state and st.session_state.quick_pay_pupil:
                    ...
                    selected = st.selectbox("Select Pupil", list(pupil_options.keys()), key="payment_pupil_fixed")
                ...
                if selected_pupil and not selected_pupil.get("is_sponsored", False):
                    ...
                    st.caption(
                        f"Class: {pupil_data['class']} | Type: {pupil_data.get('pupil_type', 'Community Child')}")

                    # Get term order for comparison
                    term_order_map = {"Term 1": 1, "Term 2": 2, "Term 3": 3}
                    current_term_num = term_order_map.get(current_term, 1)
                    ...
                    try:
                        ...
                        all_enrolled_terms = result.data

                        # FILTER: Only show terms that are PAST or CURRENT (not future)
                        # A term is valid if:
                        # - Year is less than current year (past year)
                        # - Year equals current year AND term number <= current term number
                        valid_terms = []
                        for enrollment in all_enrolled_terms:
                            term_year = enrollment["year"]
                            term_name = enrollment["term"]
                            term_num = term_order_map.get(term_name, 1)

                            if term_year < current_year:
                                valid_terms.append(enrollment)  # Past year - show all terms
                            elif term_year == current_year and term_num <= current_term_num:
                                valid_terms.append(enrollment)  # Current year, current or past terms only
                            # Future terms (year > current_year OR same year but future term) are EXCLUDED

                        if not valid_terms:
                            st.warning(f"No past or current term records found for {pupil_name}.")
                        else:
                            # Build complete fee summary ONLY for valid terms
                            term_summaries = []
                            total_fees_all_terms = 0
                            total_paid_all_terms = 0

                            for enrollment in valid_terms:
                                ...
                                # Calculate total paid (only actual payments)
                                paid_this_term = sum([p.get("amount", 0) for p in payments if p.get("amount", 0) > 0])

                                # Calculate balance (Fees - Paid)
                                # Don't use closing_balance because it might be affected by carry-forwards
                                balance = term_fee - paid_this_term
                                ...
                                if not is_active:
                                    ...
                                    status = "✅ Cleared"
                                    balance = 0  # Don't show negative (credit is shown separately)
                                else:
                            ...
                            col1, col2, col3, col4 = st.columns(4)
                            with col1:
                                st.metric("📚 Terms Completed", len(term_summaries))
                            with col2:
                                st.metric("💰 Total Fees", f"UGX {total_fees_all_terms:,.0f}")
                            with col3:
                                st.metric("✅ Total Paid", f"UGX {total_paid_all_terms:,.0f}")
                            with col4:
                                if overall_balance > 0:
                                    st.metric("⚠️ Total Outstanding", f"UGX {overall_balance:,.0f}", delta="Due")
                                else:
                                    st.metric("💳 Credit", f"UGX {abs(overall_balance):,.0f}", delta="Credit")

                            # Display term breakdown table
                            st.markdown("---")
                            st.markdown("### 📊 Term-by-Term Breakdown")

                            df_terms = pd.DataFrame(term_summaries)
                            df_display = df_terms.copy()
                            df_display["Fees (UGX)"] = df_display["Fees (UGX)"].apply(lambda x: f"UGX {x:,.0f}")
                            df_display["Paid (UGX)"] = df_display["Paid (UGX)"].apply(lambda x: f"UGX {x:,.0f}")
                            df_display["Balance (UGX)"] = df_display["Balance (UGX)"].apply(lambda x: f"UGX {x:,.0f}")

                            st.dataframe(df_display[["Term", "Fees (UGX)", "Paid (UGX)", "Balance (UGX)", "Status"]],
                            ...
                            if overall_balance <= 0:
                                ...
                                st.warning(f"⚠️ Total amount due: UGX {overall_balance:,.0f}")

                                # Show outstanding terms only
                                outstanding_terms = [t for t in term_summaries if t["Balance (UGX)"] > 0]
                                if outstanding_terms:
                                    with st.expander("📋 View outstanding terms"):
                                        for term in outstanding_terms:
                                            st.markdown(f"- **{term['Term']}**: UGX {term['Balance (UGX)']:,.0f}")
                                ...
                                if st.button("💸 Process Payment", use_container_width=True, type="primary"):
                                    if amount < 1000:
                                        ...
                                        # Sort outstanding by year and term (oldest first)
                                        outstanding_sorted = sorted(outstanding_terms,
                                                                    key=lambda x: (
                                                                    x["Year"], term_order_map.get(x["Term Name"], 0)))

                                        for term_info in outstanding_sorted:
                                            if remaining <= 0:
                                                break

                                            term_name = term_info["Term Name"]
                                            term_year = term_info["Year"]
                                            balance_due = term_info["Balance (UGX)"]

                                            pay_amount = min(remaining, balance_due)

                                            if pay_amount > 0:
                                                trans_id, new_bal, receipt, prev_bal, excess = manager.add_payment(
                                                    pupil_id, term_name, term_year, pay_amount,
                                                    f"{desc} - {term_info['Term']}"
                                                )
